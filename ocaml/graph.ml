...
let mcs g =
  ...
  let rec mcs' acc =
    if Hash_set.length v = 0
    then List.rev acc
    else (
      let select_node () =
        Hashtbl.fold lambda ~init:Temp.base ~f:(fun ~key:k ~data:v k' ->
            if phys_equal k' Temp.base
            then k
            else (
      ...
      let update_lambda node =
        ...
        Hash_set.iter nghbrs ~f:(fun nghbr ->
...
let color g nodes =
  ...
  in
  color' g nodes;
  Hashtbl.mapi result ~f:(fun ~key:_ ~data:v ->
      match v with
      | Some i -> i
      | None -> assert false)
;;
