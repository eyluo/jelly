...
let parse lexer =
  ...
  and parse_expr_prec min_prec lhs =
    ...
    match tok with
    | L.Delim | L.Eof | L.IntVal _ | L.BoolVal _ ->
      L.drop lexer;
      lhs
    | L.Operator op ->
      ...
      then lhs
        ...
        let exp = Mark.create_from_range lhs rhs (Ast.Operator (op, lhs, rhs)) in
        parse_expr_prec min_prec exp)
    | _ -> lhs
  and parse_atom () =
...
let parse_program lexer =
  let rec parse_program' prog =
    ...
    | _ ->
      ...
      parse_program' (List.append prog [ stmt ])
  in
  parse_program' []
;;
