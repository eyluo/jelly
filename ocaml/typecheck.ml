    ...
    let rec valid_exp' exp =
      ...
      | Ast.Operator (op, e1, e2) ->
        let result =
          ...
          if (v1 && v2) && Type.equal t1 op_ty1 && Type.equal t2 op_ty2
          then true, return_ty
        ...
        | Ast.Assign (sym, exp) ->
          ...
          let () =
            ...
            | Some ty ->
              ...
              if not is_valid
              then raise (TypeError "expression does not typecheck")
              else if not (Type.equal ty return_ty)
              then raise (TypeError "type of variable and expression do not match")
              else typecheck' ss
          in
          ()
        | Ast.Declare (ty, sym) ->
          ...
          if is_seen
          then raise (TypeError "redeclaration of existing variable")
          else Hashtbl.set seen ~key:sym ~data:ty;
          typecheck' ss
      in
      ()
  in
