...
let slurp fname =
  In_channel.with_file ~binary:false fname ~f:(fun ch -> In_channel.input_all ch)
;;
...
let rec next_token lxr =
  ...
  let peek_char () =
    if !(lxr.idx) = lxr.length
    then None
    else (
      let ch = lxr.file.[!(lxr.idx)] in
      Some ch)
  in
  let drop_char () =
    let incr_pos r c =
      ...
      let token =
        ...
        | 'A' .. 'Z' | 'a' .. 'z' ->
          ...
          let sym_token =
            ...
            | _ -> Symbol sym
          in
          M.create sym_token (r, c) !(lxr.pos)
        (* Parsing operators *)
        | _ ->
          let t =
            drop_char ();
            ...
            match ch with
            | '=' ->
              if is_none ch_opt || not (phys_equal '=' (Option.value_exn ch_opt))
              then Eq
              else (
                let () = drop_char () in
                Operator CompEq)
            ...
            | '&' ->
              if is_none ch_opt || not (phys_equal '&' (Option.value_exn ch_opt))
              then raise (InvalidToken (invalid_loc ()))
              else (
                let () = drop_char () in
                Operator BoolAnd)
            | '|' ->
...
let peek lxr =
  ...
  | Some tok -> tok
  | None ->
    let result = next_token lxr in
    lxr.saved_tok := Some result;
    result
;;
